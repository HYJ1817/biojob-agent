...
const ARTIFACT_PREFIX = PACKAGE_JSON.build.artifactName.split('-${version}')[0]
...
function resolveNsisPath() {
  // electron-builder NSIS artifactName comes from package.json.
...
function validateBundle() {
  ...
  // Negative assertion: the OLD expanded factory payload must not return.
  ...
  const installScript = path.join(APP.resourcesPath, 'install.ps1')
  const sourceArchive = path.join(APP.resourcesPath, 'biojob-agent-source.zip')
  if (!exists(installScript)) {
    die(`Missing bundled first-launch installer: ${installScript}`)
  }
  if (!exists(sourceArchive) || fs.statSync(sourceArchive).size < 100_000) {
    die(`Missing or invalid bundled BioJob source archive: ${sourceArchive}`)
  }
