...
async def test_bare_ping_request_produces_proper_response_and_no_stderr_noise(
    ...
    try:
        # Socket pairs provide the same full-duplex byte stream on POSIX and
        # Windows.  Anonymous os.pipe() handles cannot be registered with the
        # Windows Proactor event loop's IO completion port.
